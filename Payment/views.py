...
from Payment.forms import BillingAddress
...
def payment(request):
    ...
    mypayment.set_customer_info(name=current_user.profile.full_name,
                                ...
                                address1=current_user.profile.address_line_1, 
                                address2=current_user.profile.address_line_2, 
...
def complete(request):
    if request.method == 'POST' or request.method == 'post':
        ...
        if status == 'VALID':
            ...
            return redirect("Payment:purchase", val_id,tran_id,payment_data)
        
        elif status == 'FAILED':
            messages.warning(request, f"Your Payment Failed! Please Try Again! Page will be redirected!")
            
        else:
            messages.warning(request, f"Your Payment Canceled! Please Try Again! Page will be redirected!")
    return render(request, "Payment/complete.html",)


@login_required
def purchase(request, val_id, tran_id,payment_data):
    order_qs = Order.objects.filter(user=request.user, ordered=False)
    order = order_qs[0]
    orderId = tran_id
    order.ordered = True
    ...
    order.full_payment_data = payment_data
    order.save()
